/* train_reservation.c
   Simple Train Ticket Reservation System in C
   Compile: gcc -o train_reservation train_reservation.c
*/
...
typedef struct {
    ...
    char gender[10];
    int seat_no;
    int active; /* 1=booked, 0=cancelled */
} Booking;

/* Global arrays (simple program) */
...
void init_trains() {
    /* Sample trains - you can modify/add */
    trains[0].train_id = 101; strcpy(trains[0].name, "Coastal Express"); strcpy(trains[0].source, "Mumbai"); strcpy(trains[0].dest, "Goa"); trains[0].total_seats = 50; trains[0].seats_booked = 0;
    trains[1].train_id = 102; strcpy(trains[1].name, "Mountain Flyer");   strcpy(trains[1].source, "Delhi"); strcpy(trains[1].dest, "Shimla"); trains[1].total_seats = 40; trains[1].seats_booked = 0;
    trains[2].train_id = 103; strcpy(trains[2].name, "Desert Rider");    strcpy(trains[2].source, "Jaipur"); strcpy(trains[2].dest, "Jodhpur"); trains[2].total_seats = 60; trains[2].seats_booked = 0;
    trains[3].train_id = 104; strcpy(trains[3].name, "Riverine Line");   strcpy(trains[3].source, "Kolkata"); strcpy(trains[3].dest, "Puri"); trains[3].total_seats = 55; trains[3].seats_booked = 0;
    trains[4].train_id = 105; strcpy(trains[4].name, "Southern Star");   strcpy(trains[4].source, "Bangalore"); strcpy(trains[4].dest, "Chennai"); trains[4].total_seats = 45; trains[4].seats_booked = 0;
}
...
void load_bookings_from_file() {
    ...
    /* Recalculate seats_booked for trains from loaded bookings */
...
int find_free_seat_number(Train *t) {
    /* find a seat number between 1 and total_seats not already used by active bookings */
